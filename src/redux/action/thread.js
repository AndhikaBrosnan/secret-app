...
import arrayOfAvatars from "../../resources/avatar-icon/avatars";
...
export const postThread = async (anon, thread, auth) => {
  const timeElapsed = Date.now();
  const today = new Date(timeElapsed);

  //check if anonymous is checked
  let newPost =
    anon === false
      ? {
      ...
      : {
          avatar:
            arrayOfAvatars[Math.floor(Math.random() * arrayOfAvatars.length)],
          name: "Anonymous",
          text: thread,
          likes: 0,
          createdAt: today.toDateString(),
